...
def update_train_log(data_shape, eval_test, runtime, MODEL_VERSION, MODEL_VERSION_NOTE, MODEL_NAME, test=False, hyperparameters=None):
    ...
    header = ['unique_id','timestamp','x_shape','eval_test','model_version',
              'model_version_note','runtime', "name", "hyperparameters"]
    write_header = False
    ...
    with open(logfile, 'a') as csvfile:
        ...
        to_write = map(str, [uuid.uuid4(), time.time(), data_shape, eval_test,
                            MODEL_VERSION, MODEL_VERSION_NOTE, runtime, MODEL_NAME, hyperparameters])
...
if __name__ == "__main__":
    ...
    for i in range(len(model.MODELS)):
        update_train_log(str((100,10)),"{'rmse':0.5}","00:00:01", model.MODEL_VERSIONS[i], model.MODEL_NOTES[i], model.MODELS[i], test=True, hyperparameters=True)
    ## predict logger
